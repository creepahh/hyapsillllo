...
Install nothing extra. From this folder, run:

```sh
npm run build
```

This creates `dist/hyapsilo-notes/` for loading unpacked and
`dist/hyapsilo-notes.zip` for sharing or uploading to the Chrome Web Store.
To test the built folder, choose `dist/hyapsilo-notes/` in **Load unpacked**.
